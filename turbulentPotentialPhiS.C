...
#include "turbulentPotentialPhiS.H"
...
defineTypeNameAndDebug(turbulentPotentialPhiS, 0);
addToRunTimeSelectionTable(RASModel, turbulentPotentialPhiS, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> turbulentPotentialPhiS::Ts() const
{ 
	if(tslimiter_ == "true")
	{
...
tmp<volScalarField> turbulentPotentialPhiS::Ls() const
{
...
turbulentPotentialPhiS::turbulentPotentialPhiS
(
...
tmp<volSymmTensorField> turbulentPotentialPhiS::R() const
{
...
// Not used but necessary for RAS Model
tmp<volSymmTensorField> turbulentPotentialPhiS::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
bool turbulentPotentialPhiS::read()
{
    if (RASModel::read())
    {
...
void turbulentPotentialPhiS::correct()
{
